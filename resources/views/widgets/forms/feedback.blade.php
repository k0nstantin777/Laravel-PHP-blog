...
@else
    ...
    <input id="sub" name="subject" placeholder="Тема сообщения" value="{{ old('subject') }}" {{$errors->has('subject') ? 'class=error':'' }} />
    ...
    <textarea id="body" name="message" placeholder="Текст сообщения" {{$errors->has('message') ? 'class=error':'' }}  >{{ old('message') }}</textarea>
